...
Across four workload scenarios with five independent runs, AIF achieves 86.0\% average SLO satisfaction without any training data, compared to 87.8\% for the threshold-based heuristic, 89.9\% for the myopic oracle, and 88.2\% for DQN (which requires per-scenario training).
A sensitivity analysis of the AIF generative model reveals that correcting the OFFLOAD likelihood from per-stream to system-level semantics improves SLO compliance by 8 percentage points, demonstrating that model specification---not algorithmic complexity---is the primary determinant of AIF performance.
...
This occurs because the SKIP likelihood model ($A^{\textsc{Skip}} = [0.5, 0.5, 0.0]$) treats SKIP as observationally neutral, making it insufficiently costly in the EFE computation.
This is a clear target for model refinement (\Cref{sec:ablation}).
...
Unlike DQN, which requires 5 training episodes per scenario and still produces seed-dependent policies (burst std = 9.7\%), AIF operates from the first step using profiling-derived parameters.
...
\textbf{Excessive SKIP usage.}
AIF's skip ratio (9.3--16.3\%) is substantially higher than other controllers, reducing detection coverage.
This arises from the uninformative SKIP likelihood ($[0.5, 0.5, 0.0]$), which does not penalize SKIP in the EFE computation.
A more accurate SKIP model---reflecting that skipping yields zero detections and should be treated as a negative outcome---would likely reduce this behavior.
...
The ablation hierarchy (OFFLOAD likelihood: 7.9 pp $>$ precision: 3.6 pp $>$ epistemic weight: 2.7 pp $>$ cooldown: 1.5 pp on SLO) reveals that for this real-world scheduling problem, AIF performance is primarily determined by the causal semantics of the likelihood model, not by the fine-tuning of the EFE computation or anti-oscillation hacks.
...
AIF achieves 86.0\% average SLO satisfaction without training data, compared to 87.8\% for the threshold-based heuristic, 89.9\% for the myopic oracle, and 88.2\% for DQN (which requires per-scenario training and exhibits high cross-run variance).
Crucially, we demonstrated that the generative model's OFFLOAD likelihood specification---modeling system-level effects rather than per-stream metrics---accounts for an 8-percentage-point SLO improvement, making model construction the primary practical challenge for AIF deployment.
The ablation hierarchy further reveals that likelihood semantics dominate over precision tuning, epistemic weighting, and cooldown scheduling, providing clear guidance for practitioners.
...
% Time-series plots and belief evolution figures to be included after generation.
